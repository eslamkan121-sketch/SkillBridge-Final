...
function uiText(lang: 'en' | 'ar') {
  ...
  return {
    ...
    newChatConfirm: s(
      'Start a new chat with {name}? This clears the current conversation.',
      'تبدأ محادثة جديدة مع {name}؟ ده بيمسح المحادثة الحالية.',
    ),
    clearChat: s('Clear Chat', 'امسح المحادثة'),
    clearChatConfirm: s(
      'Clear {name}\'s chat? This permanently deletes it.',
      'تمسح محادثة {name}؟ ده حذف نهائي.',
    ),
